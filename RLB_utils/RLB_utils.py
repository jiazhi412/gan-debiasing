...
def is_converge(mi_list, opt):
    ...
    region1 = np.array([mi_list[i] for i in range(-window_size//2,0)]) # back
    region2 = np.array([mi_list[i] for i in range(-window_size, -window_size//2)]) # front
...
def count(input,opt):
    a = torch.squeeze(input)
    # print(a.tolist())
    distribution = Counter(a.tolist())
    # print(distribution)
